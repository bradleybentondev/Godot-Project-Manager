...
pub fn project_reconciliation(existing_projects: Vec<ProjectData>, found_projects: Vec<ProjectData>, all_godot_versions: Vec<GodotEngineVersion>) -> Vec<ProjectData> {
...
pub fn validate_godot_versions(projects: &mut Vec<ProjectData>, godot_versions: Vec<GodotEngineVersion>){
    let mut godot_versions_iter = godot_versions.iter();
    for project in projects {
        let godot_version = godot_versions_iter.find(|godot| godot.version_name == project.project_version );
        project.engine_valid = godot_version.is_some() && PathBuf::from(&godot_version.unwrap().path).exists();
    }
}
